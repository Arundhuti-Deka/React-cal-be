...
export default class UserController{
    ...
    public authenticate = async (req: Request, res: Response) =>{
        try{
            const user: any = await userModel.findOne({$or: [{email: req.body.email}, {phoneNo: req.body.phoneNo}],  isActive: true});
            if(user) {
    ...
    public userDisable = async (req: any, res: Response) => {
        try{
            const user: any = await userModel.findById(req['tokenId']);
            if(user){
                let userUpdate = await userModel.updateOne({_id: req['tokenId']},{$set: {isActive: false}}, {new : true});
                if(userUpdate){
